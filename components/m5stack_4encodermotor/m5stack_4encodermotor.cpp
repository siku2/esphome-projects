...
void M5Stack4EncoderMotor::setup() {
  ...
  if (this->fw_version_ == 0) {
    ...
    ESP_LOGW(
        TAG,
        "M5Stack 4 Encoder Motor FW version is %u. Consider updating to version 4 since it contains important fixes.",
        this->fw_version_);
  }
...
void M5Stack4EncoderMotor::set_motor_pwm_duty(Motor motor, int8_t duty) { this->reg(REG_MOTOR_PWM_DUTY(motor)) = duty; }
